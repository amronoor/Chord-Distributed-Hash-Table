...
void put(string key,string value,NodeInformation &nodeInfo){
	if(key == "" || value == ""){
		cout<<"Key or value field empty\n"; //check if the key and value are empty or not 
		return;
	}

	else{

        lli keyHash = help.getHash(key);// produce the hashcode for the key
        cout<<"********************************************"<<endl;
...
void get(string key,NodeInformation nodeInfo){
    ...
    if(key == ""){
        ...
        if(val == "")
            cout<<"Key Not found\n";// if there is no value then the key is not found 
...
void create(NodeInformation &nodeInfo){
    ...
    string key = ip+":"+(to_string(port)); //to concatinate the ip and port we convert the port to string  
    
    lli hash = help.getHash(key);

    /* setting id, successor , successor list , predecessor ,finger table and status of node */
    nodeInfo.setId(hash); //the address on the ring 
...
/* join in a Chord DHT ring */
void join(NodeInformation &nodeInfo,string ip,string port){

    /*if there is no active node on this ip or port */
...
void printState(NodeInformation nodeInfo){
    ...
    cout<<"Pred "<<pre.first.first<<" "<<pre.first.second<<" "<<pre.second<<endl;
        cout<<"****************"<<endl;
        cout<<"The Finger Table"<<endl;
        cout<<"****************"<<endl;
        cout<<"Fingers"<< "             "<<"ID's"<< "                "<<"IP's"<< "         "<<"Ports"<<endl;
    
    for(int i=1;i<=M;i++){
        ip = fingerTable[i].first.first;
        port = fingerTable[i].first.second;
        id = fingerTable[i].second;
        cout<<"Finger["<<i<<"] : "<<"ID :["<<id<<"] : "<<"IP :["<<ip<<"] : "<<"Port :["<<port<<"]"<<endl;
    }
        cout<<"-------------------------------------------------------------------"<<endl;
        cout<<"-------------------------------------------------------------------"<<endl;
        
        cout<<"*******************"<<endl;
        cout<<"The Successors List"<<endl;
        cout<<"*******************"<<endl;
        cout<<"Successors"<<"        " <<"SuccessorsID's"<<"          " <<"SuccessorIP's" <<"      " <<"successorsPorts"<<endl;
    
    for(int i=1;i<=R;i++){
        ip = succList[i].first.first;
        port = succList[i].first.second;
        id = succList[i].second;
        cout<<"Successor["<<i<<"] "<<"SuccID :["<<id<<"] "<< "SuccIP :[" << ip<<"] "<<"SuccPort :[" <<port<<"] "<<endl;
    }
        cout<<"--------------------------------------------------------------------"<<endl;
        cout<<"--------------------------------------------------------------------"<<endl;
}
