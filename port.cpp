...
void SocketAndPort::specifyPortServer(){
	...
	srand(time(0));
	portNoServer = rand() % 65536;// the modulation is for staying in the loop and don't exceeds 65536
...
void SocketAndPort::changePortNumber(int newPortNumber){
	if(newPortNumber < 1024 || newPortNumber > 65535){
		...
		if( portInUse(newPortNumber) ){
			...
			close(sock);
			socklen_t len = sizeof(current);
			sock = socket(AF_INET,SOCK_DGRAM,0); /*AF_INET it's a connectionless address familly socket 
												used to designate the type of addresses that your socket can communicate with */

			current.sin_port = htons(newPortNumber); /*The htons() function converts the unsigned short integer 
